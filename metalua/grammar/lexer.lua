...
---@class lexer
---@field alpha table<string,boolean>
...
function M.new_position(line, column, offset, source)
   ...
   return setmetatable({ line = line, column = column, offset = offset, source = source, id = id }, MT.position)
end
...
function MT.position_factory:get_position(offset)
   ...
   if offset < line2offset[left] then
      left = 1
   end
   local right = left + 1
   if line2offset[right] <= offset then
      right = right + 1
   end
   if line2offset[right] <= offset then
      right = #line2offset
   ...
   while true do
      ...
      if line2offset[middle] <= offset then
         left = middle
      else
         right = middle
      end
...
function MT.lineinfo:__tostring()
   ...
   local line = fli.line
   if line ~= lli.line then
      line = line .. "-" .. lli.line
   end
   local column = fli.column
   if column ~= lli.column then
      column = column .. "-" .. lli.column
   end
   local offset = fli.offset
   if offset ~= lli.offset then
      offset = offset .. "-" .. lli.offset
   end
   return string.format(
      "<%s%s|L%s|C%s|K%s%s>",
...
---Return the text of the comment, as a string.
---@return string
...
---@table Patterns used by [lexer :extract] to decompose the raw string into correctly tagged tokens.
lexer.patterns = {
   spaces = "^[ \r\n\t]*()",
   short_comment = "^%-%-([^\n]*)\n?()",
   --final_short_comment = "^%-%-([^\n]*)()$",
...
local function unescape_string(s)
   ...
   local function unesc_digits(backslashes, digits)
      ...
      if c == "\\" then
         c = "\\\\"
      end -- parsed by unesc_letter (test: "\092b" --> "\\b")
   ...
   local function unesc_hex(backslashes, digits)
      ...
      if c == "\\" then
         c = "\\\\"
      end -- parsed by unesc_letter (test: "\x5cb" --> "\\b")
      return backslashes .. c
   end
      ...
      for _, extractor in ipairs(self.extractors) do
         ...
         if tag then
            ...
            if tag == "Comment" then
               ...
               if
                   not xtra                                            -- new comment is short
                   and prev_comment
                   and not prev_comment[2]                             -- prev comment is short
                   and prev_comment.lineinfo.last.line + 1 == fli.line
               then                                                    -- adjascent lines
...
function lexer:extract_number()
   ...
   if j then
      j = self.src:match(self.patterns.number_mantissa_hex[1], j)
          or self.src:match(self.patterns.number_mantissa_hex[2], j)
      if j then
         j = self.src:match(self.patterns.number_exponant_hex, j) or j
      end
   else
      j = self.src:match(self.patterns.number_mantissa[1], self.i)
          or self.src:match(self.patterns.number_mantissa[2], self.i)
      if j then
   ...
   if not n then
      error(str .. " is not a valid number according to tonumber()")
   end
   self.i = j
   return "Number", n
end
...
function lexer:add(w)
   if type(w) == "table" then
      ...
      if w:match(self.patterns.word .. "$") then
         self.alpha[w] = true
...
function lexer:sync()
   ...
   if p1 then
      ...
      if li_first.comments then
         li_first = li_first.comments.lineinfo.first
      end
...
function lexer:is_keyword(a, ...)
   if not a or a.tag ~= "Keyword" then
      return false
   end
   local words = { ... }
   if #words == 0 then
...
function lexer:check(...)
   ...
   local a = self:next()
   local function err()
      error("Got " .. tostring(a) .. ", expected one of these keywords : '" .. table.concat(words, "', '") .. "'")
   end
   if not a or a.tag ~= "Keyword" then
      err()
   end
...
function lexer:clone()
   ...
   for word in pairs(self.alpha) do
      alpha_clone[word] = true
   end
   for letter, list in pairs(self.sym) do
      sym_clone[letter] = { unpack(list) }
   end
...
---Cancel everything left in a lexer, all subsequent attempts at `:peek()` or `:next()` will return `Eof`.
---@return nil
function lexer:kill()
   self.i = #self.src + 1
   self.peeked = {}
